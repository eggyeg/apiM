...
export function CompareVersions({
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={`relative flex h-full max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-border-light bg-bg-secondary shadow-[0_28px_80px_rgba(0,0,0,0.6)] transition-all duration-150 ${
          visible ? "scale-100 opacity-100" : "scale-[0.98] opacity-0"
        }`}
      >
        <header className="flex flex-none items-center justify-between gap-3 border-b border-border px-4 py-2.5">
          ...
          <div className="flex items-center gap-1.5">
            {canPage && (
              <>
                <button
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-lg text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary disabled:opacity-30"
                >
            ...
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-lg text-text-secondary transition-colors hover:bg-danger/15 hover:text-danger"
            >
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-none items-center gap-2 px-4 py-1.5">
        ...
        {version.model && (
          <span className="text-[11px] text-text-muted">{version.model}</span>
        )}
      </div>

      {/* Independent scroll, contained so the page behind stays put. */}
      <div className="prose-chat min-h-0 flex-1 overflow-y-auto px-4 pb-4 text-[15px] leading-relaxed text-text-primary [overscroll-behavior:contain]">
