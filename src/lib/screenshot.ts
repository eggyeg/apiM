...
import { asciiOnly, writePowerShellScript } from "@/lib/hidden-display";
...
# Generated by apiM on every capture. Edits here are overwritten.
...
export async function captureWindow(
  ...
  if (process.platform === "win32") {
    ...
    // BOM, always: a BOM-less script is decoded with the ANSI code page by
    // Windows PowerShell 5.1, and one smart-quote-shaped byte turns a working
    // script into a parse error twenty lines below the real cause.
    await writePowerShellScript(scriptPath, script);
    const out = run(
      "powershell",
      [
        "-NoProfile",
