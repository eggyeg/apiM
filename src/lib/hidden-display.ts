...
export function hiddenLaunchScript(): string {
  return asciiOnly(String.raw`
# Generated by apiM on every hidden launch. Edits here are overwritten.
...
/**
 * PowerShell scripts must be pure ASCII. This is not a style rule.
 *
 * The bug it exists to prevent, in full, because it cost a whole evening and
 * was diagnosed as a platform fault:
 *
 *   An em dash (U+2014) sat inside a double-quoted error message. The file
 *   was written UTF-8 with no BOM. Windows PowerShell 5.1 reads a BOM-less
 *   file using the ANSI code page, so those three bytes (E2 80 94) decoded as
 *   cp1252 "a-circumflex, euro, RIGHT DOUBLE QUOTATION MARK". PowerShell
 *   accepts U+201D as a real string delimiter — so the string ENDED in the
 *   middle of the sentence, the rest of the line became loose tokens, and the
 *   parser finally gave up at the closing brace twenty lines later:
 *   "ParserError ... line 60 char 1".
 *
 *   Which is why rewriting the script by hand changed nothing and produced a
 *   byte-identical error from a different file: apiM regenerates this script
 *   on every launch, so the hand-written one was overwritten before it ran.
 *   Two files, one error, one cause — this one.
 *
 * Both halves are fixed: the text is ASCII, and the file is written WITH a
 * BOM so the decoding is never in doubt. This function is the belt to that
 * pair of braces, and a test asserts every generated script passes it.
 */
export function asciiOnly(script: string): string {
  return script
    .replace(/[\u2013\u2014]/g, "-")
    .replace(/[\u2018\u2019]/g, "'")
    .replace(/[\u201c\u201d]/g, '"')
    .replace(/\u2026/g, "...")
    .replace(/\u00a0/g, " ")
    // Anything still non-ASCII would be a silent decoding hazard, so it goes.
 ...
 *
 * The BOM is the whole point: without it, 5.1 decodes as the ANSI code page.
 * With it, the file is unambiguously UTF-8 no matter what ends up in the
 * text later.
 */
export async function writePowerShellScript(
  ...
  await fs.writeFile(filePath, "\ufeff" + asciiOnly(script), "utf8");
}
...
> {
  ...
  if (active?.kind === "windows-desktop") {
    ...
    await writePowerShellScript(scriptPath, hiddenLaunchScript());
    return {
      ok: true,
      command: "powershell",
      args: [
