...
const LIMIT_LANGUAGE =
  /\b(?:hit|reached|hitting|ran out of) (?:the |my |an )?(?:token |context |output |length |character )?limit\b/i;
const LIMIT_LANGUAGE_EXTRA =
  /\b(?:context window|token budget|max tokens|output limit|I(?:'ll| will) have to stop|I(?:'m| am) (?:stopping|pausing) (?:here|for now)|stop(?:ping)? here(?: for now)?|continue in (?:the )?(?:next|another) (?:message|reply|turn)|to be continued|say (?:\"|')?(?:continue|resume)|type (?:\"|')?(?:continue|resume)|ask me to (?:continue|resume)|I cannot continue|pick this up)\b/i;
...
export function detectPrematureStop(
  ...
  // No tools and no leftover plan: this is a chat answer, not a task.
  if (input.toolRounds === 0) return null;
